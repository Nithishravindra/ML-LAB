...
def entropy(headers, data, attribute='PlayTennis', gain=False):
    ...
    for Class, prob in zip(cnt.keys(), probs):
        gain += -prob * \
            entropy(headers, split_data(headers, data, attribute, Class))
    return gain
...
def information_gain(headers, data):
    ...
    for attribute in headers:
        if attribute == 'PlayTennis':
            continue
        gain = entropy(headers, data) + entropy(headers,
                                                data, attribute, gain=True)
        if gain > max_gain:
            max_gain = gain
...
def id3(root):
    if len(root.headers) == 1:
        ...
        return
    outcome_value_set = set(get_attribute_column(
        root.headers, root.data, 'PlayTennis'))
    ...
    for attr_val in set(get_attribute_column(root.headers, root.data, max_gain_attribute)):
        child_data = split_data(root.headers, root.data,
                                max_gain_attribute, attr_val)
        if child_data is None or len(child_data) == 0:
            root.decision = most_common_outcome(root.headers, root.data)
            return
        (new_headers, new_data) = drop_attribute(
            root.headers, child_data, max_gain_attribute)
...
def print_tree(root, disp=""):
    ...
    for attribute, node in root.child.items():
        print_tree(
            node, disp + "IF {} EQUALS {} AND ".format(root.decision_attribute, attribute))
